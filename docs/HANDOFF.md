...
- **Progress (updated as epics complete):** FND (12), DB (5), TEN (12), DSN (9), JOB (8), CFG (14), STO (6), ING (14), PRC (14), REV (16), CAN (4), EXP (10 of 11), and AIO (15 of 19) are DONE — implemented, tested, committed to `dev`, CI green. (REV-016's human study runs during PIL. EXP-011 is P1, blocked on the pilot-ERP-choice ADR — an OWNER decision. AIO deferred items: AIO-005 local layout OCR is P1 with a heavy isolated dependency and a benchmark gate; AIO-006 hosted OCR is blocked on OPEN-003 (managed-OCR provider decision — OWNER); AIO-008/009 hosted extraction adapters are blocked on OPEN-004 (production provider decision — OWNER) — the AIO-001 contracts, AIO-011 request builder, and AIO-012 repair policy mean each lands as an adapter + registration, no pipeline change. Known wiring gaps stated in commits: the provider router and evaluation gate are libraries awaiting per-stream config resolution and evaluation-run persistence; the simulation endpoint and provider health honestly report those gaps.) Next action: `CAT-001` in `docs/BUILD_BACKLOG.md`, then the rest of CAT in order.
