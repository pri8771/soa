...
- **duplicate_policy is honored end to end**: `allow` no longer injects `meta.duplicate_of` into rule evaluation (previously ANY duplicate routed to review via the hardcoded `duplicates.business_hook` baseline rule regardless of policy). `DuplicatePolicy` moved to `soa_db` (worker + API share it); the worker resolves the run's PINNED stream config via a read-only `stream_versions` projection. The seeded uploads stream is on published v2 with `duplicate_policy: allow`; `seed_local.py` defaults it for fresh setups. Business-level duplicates (same PO, different bytes) are still NOT detected — the ING-006 hook stays dormant.
...
- **Known gaps confirmed still open** (verified against code/DB this session): outbox events are written but never relayed (4 stuck `pending`); approval creates no export job (approve leads nowhere — ERP adapters unreachable from the real flow); usage ledger never written (analytics/billing read zeros); retention engine unscheduled; local Postgres role is a superuser so RLS is bypassed locally (CI's non-superuser job is the only RLS exercise); worker is single-claim sequential.
