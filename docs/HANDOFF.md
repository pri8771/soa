...
- **Progress (updated as epics complete):** FND (12), DB (5), TEN (12), DSN (9), JOB (8), CFG (14), STO (6), ING (14), PRC (14), REV (16), CAN (4), and EXP (10 of 11) are DONE — implemented, tested, committed to `dev`, CI green. (REV-016's study protocol is written — `docs/REVIEW_USABILITY_BENCHMARK.md` — but the human study itself runs during PIL; its release gate stays open until then. EXP-011, the first production ERP adapter, is P1 and blocked on the pilot-ERP-choice ADR — an OWNER decision — plus destination sandbox access; the EXP-010 adapter contract means it lands as a registration + the shared contract suite, no orchestration change.) Next action: `AIO-001` in `docs/BUILD_BACKLOG.md`, then the rest of AIO in order.
