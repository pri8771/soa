...
the tenant context, injects the process-lifetime delivery dependencies (the
HTTP client, the SSRF egress allowlist, the secret store), stamps the attempt
time, and lets the executor bind RLS separately for each short database unit
of work.
