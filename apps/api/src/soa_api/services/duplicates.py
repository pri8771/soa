...
Concurrency: intake acquires a transaction-scoped PostgreSQL advisory
lock keyed by tenant + stream + content hash before registering a
document. The lock is held through the caller's commit, so a concurrent
delivery of the same bytes cannot run its duplicate lookup until the
first document is visible. Different tenants, streams, or hashes remain
independent.
"""
...
async def lock_exact_duplicate_intake(
    ...
    *,
    ...
    """Serialize exact-duplicate registration until the transaction ends."""
